const en = {
  common: {
    back: 'Go back',
  },
  tabs: {
    timeline: 'Timeline',
    shelf: 'My Shelf',
  },
  timeline: {
    finishedReading: 'Finished reading',
    emptyTitle: 'Connect with friends',
    emptyBody: 'Reading records from your friends will appear here.',
    emptyBookmarksTitle: 'No bookmarks yet',
    emptyBookmarks: 'No bookmarks yet. Tap the bookmark icon on a card to save it.',
    loadErrorTitle: "Couldn't load",
    loadErrorBody: 'Something went wrong. Please try again.',
    modalViewProfile: 'View profile',
    modalClose: 'Close',
    bookmarkAdd: 'Save bookmark',
    bookmarkRemove: 'Remove bookmark',
    filterLabel: 'Bookmarks',
    filterBookmarks: 'Show bookmarks only',
    filterAll: 'Show all',
  },
  addFriend: {
    heading: 'Add a friend',
    placeholder: "Enter your friend's ID",
    search: 'Search',
    notFound: 'No user with that ID.',
    selfMatch: "That's your own ID.",
    viewProfile: 'View profile',
    close: 'Close',
  },
  bookSearch: {
    placeholder: 'Search by title or author',
    button: 'Search',
    noResults: 'No results found',
    unknownAuthor: 'Unknown author',
    searchError: 'Search failed. Please try again.',
    rateLimitError: 'Too many searches. Please wait a moment.',
    scanBarcode: 'Scan barcode',
  },
  bookScan: {
    hint: 'Point the camera at a book barcode.',
    looking: 'Looking up the book…',
    notFound: 'We couldn’t find this book. Try a title search instead.',
    lookupError: 'Lookup failed. Try a title search instead.',
    permissionTitle: 'Camera access is off',
    permissionBody: 'Yomoyo needs camera access to scan barcodes. You can still search by title.',
    searchByTitle: 'Search by title',
    tryAgain: 'Try again',
  },
  bookDetail: {
    unknownAuthor: 'Unknown author',
    goToShelf: 'View in shelf',
  },
  shelf: {
    finished: 'Finished',
    finishedCountInline_one: '({{count}})',
    finishedCountInline_other: '({{count}})',
    emptyTitle: 'Record your first book',
    emptyFinished: 'No finished books yet',
    addBook: 'Add a finished book',
    markAsFinished: 'Mark as Finished',
    yourIdTitle: 'Your ID',
    copyId: 'Copy my ID',
    idCopied: 'Copied!',
    changeAvatar: 'Change avatar',
    editName: 'Edit name',
    closeAvatarPicker: 'Close',
    saveName: 'Save',
    cancelNameEdit: 'Cancel',
    saveError: 'Could not save. Please try again.',
  },
  settings: {
    languageTitle: 'Language',
    themeTitle: 'Appearance',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeSystem: 'System',
    openSettings: 'Open settings',
    logout: 'Log out',
    logoutError: 'Logout failed. Please try again.',
  },
  userProfile: {
    follow: 'Receive updates',
    unfollow: 'Stop receiving',
    notFound: 'This page does not exist.',
    emptyBooks: 'No finished books yet',
    ownPageNote: 'This is your page',
    followSuccess: "You'll now see {{name}}'s reading on your Timeline.",
    backToTimeline: 'Back to Timeline',
    finishedCount_one: '{{count}} book finished',
    finishedCount_other: '{{count}} books finished',
    history: {
      weekTileLabel_zero: 'No books finished this week',
      weekTileLabel_one: '1 book finished this week',
      weekTileLabel_other: '{{count}} books finished this week',
    },
  },
  profile: {
    displayName: {
      label: 'Display name',
      placeholder: 'Enter your name',
      errors: {
        empty: 'Please enter a name.',
        tooLong: 'Name must be 20 characters or fewer.',
        newline: 'Line breaks are not allowed.',
      },
    },
    edit: {
      title: 'Edit profile',
      save: 'Save',
      cancel: 'Cancel',
    },
    animals: {
      seal: 'Seal',
      chicken: 'Chicken',
      buffalo: 'Buffalo',
      cow: 'Cow',
      monkey: 'Monkey',
      fox: 'Fox',
      wolf: 'Wolf',
      raccoon_dog: 'Raccoon dog',
      sheep: 'Sheep',
      ladybug: 'Ladybug',
      rhinoceros_beetle: 'Rhinoceros beetle',
      deer: 'Deer',
      bulldog: 'Bulldog',
      bear: 'Bear',
      eagle: 'Eagle',
      penguin: 'Penguin',
    },
  },
  onboarding: {
    heading: 'Yomoyo',
    concept: "When someone you follow finishes a book, you'll know.",
    introHeading: 'A quiet place for friends who read',
    introBody:
      "Yomoyo isn't a public feed. When a friend you follow finishes a book, a gentle note arrives — nothing more.",
    signInWithGoogle: 'Sign in with Google',
    signInWithApple: 'Sign in with Apple',
    signInError: 'Sign-in failed. Please try again.',
    avatarHeading: 'Pick an avatar that feels like you',
    avatarContinue: 'Continue',
    notificationHeading: 'A quiet “I just finished a book” arrives',
    notificationBody: 'Only when a friend finishes a book — gently.',
    receiveHighlightNotify: 'Get ready to receive notes',
    receiveHighlightConnect: 'Connect with friends you like',
    allowButton: 'Receive notes',
    skipLink: 'Set up later',
    sendHeading: "Your finished book, someone's quiet joy",
    sendHighlightShareId: 'Share your ID with friends',
    sendHighlightRecord: 'Record the books you finish',
    sendButton: 'Start here',
    progressLabel: 'Step {{current}} of {{total}}',
  },
} as const;

export default en;
